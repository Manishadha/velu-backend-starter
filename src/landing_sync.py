...
def _clean_copy(text: str | None) -> str:
    value = (text or "").strip()
    if not value:
        return ""
    for frag in BAD_SNIPPETS:
        value = value.replace(frag, "")
...
def _generate_index_tsx(spec: Dict[str, Any], summary: str | None) -> str:
    raw_module = spec.get("module_name") or "app"
    raw_product_type = spec.get("product_type", "product")
    raw_goal = spec.get("goal", "")
    raw_features = spec.get("main_features") or []
    raw_pages = spec.get("pages") or []
    raw_target_users = spec.get("target_users", "")
    raw_design_style = spec.get("design_style", "")
    frontend = spec.get("frontend", "nextjs")
    backend = spec.get("backend", "fastapi")
    database = spec.get("database", "sqlite")

    module = _clean_copy(str(raw_module))
    product_type = _clean_copy(str(raw_product_type))
    goal = _clean_copy(str(raw_goal))
    target_users = _clean_copy(str(raw_target_users))
    design_style = _clean_copy(str(raw_design_style))

    features: list[str] = []
    for f in raw_features:
        clean = _clean_copy(str(f))
        if clean:
            features.append(clean)

    pages: list[str] = []
    for p in raw_pages:
        clean = _clean_copy(str(p))
        if clean:
            pages.append(clean)
